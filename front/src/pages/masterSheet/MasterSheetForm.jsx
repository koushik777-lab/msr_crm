...
const MasterSheetForm = ({ onSuccess, editRecord, onCancelEdit, isPublic = false }) => {
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                if (editRecord) {
                    await axios.put(
                        `${API_URI}/master-sheet/${editRecord._id}`,
                        values,
                        getHeaders()
                    );
                } else {
                    await axios.post(
                        `${API_URI}/master-sheet${isPublic ? '/public' : ''}`,
                        values,
                        isPublic ? undefined : getHeaders()
                    );
                }
    ...
    useEffect(() => {
        if (editRecord) {
            ...
            // Prevent React warnings about uncontrolled inputs
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                        <TextField
                            ...
                            value={formik.values.accreditationBody}
                            onChange={formik.handleChange}
                            variant="outlined"
                            size="small"
                        >
                    ...
                    <Grid item xs={12} md={6}>
                        <TextField
                            ...
                            size="small"
                        >
                            {CERTIFICATION_BODIES.map((option) => (
                                <MenuItem key={option} value={option}>
                                    {option}
                                </MenuItem>
                            ))}
                    ...
                    <Grid item xs={12} md={3}>
                        <TextField
                            ...
                            InputLabelProps={{ shrink: true }}
                            value={formik.values.firstSurvDate}
                            onChange={formik.handleChange}
                            size="small"
                        />
                    </Grid>
                    <Grid item xs={12} md={3}>
                        <TextField
                            ...
                            InputLabelProps={{ shrink: true }}
                            value={formik.values.secondSurvDate}
                            onChange={formik.handleChange}
                            size="small"
                        />
                    </Grid>
                    ...
                    <Grid item xs={12}>
                        <Divider sx={{ my: 1 }}>Company Details</Divider>
                    </Grid>
                    ...
                    <Grid item xs={12} md={4}>
                        <TextField
                            ...
                            value={formik.values.marketingExecutive}
                            onChange={formik.handleChange}
                            size="small"
                        />
                    </Grid>
                    <Grid item xs={12} md={4}>
                        <TextField
                            ...
                            label="Courier Date"
                            InputLabelProps={{ shrink: true }}
                            value={formik.values.courierDate}
                            onChange={formik.handleChange}
                            size="small"
                        />
                    </Grid>
                    ...
                    <Grid item xs={12}>
                        <Divider sx={{ my: 1 }}>Financial & Client Details</Divider>
                    </Grid>
                    ...
                    <Grid item xs={12} md={4}>
                        <TextField
                            ...
                            value={formik.values.clientConsultant}
                            onChange={formik.handleChange}
                            size="small"
                        >
                            {CLIENT_CONSULTANT_OPTIONS.map((option) => (
                                <MenuItem key={option} value={option}>
                                    {option}
                                </MenuItem>
                            ))}
                    ...
                    <Grid item xs={12} sx={{ mt: 3, display: 'flex', gap: 2 }}>
                        ...
                        {editRecord ? (
                            <Button
                                color="inherit"
                                variant="outlined"
                                onClick={() => { formik.resetForm(); if (onCancelEdit) onCancelEdit(); }}
                                sx={{ px: 4, py: 1, borderRadius: 2, fontWeight: 'bold' }}
                            >
                                Cancel Edit
                            </Button>
