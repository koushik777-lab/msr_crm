...
import { Box, Typography, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, TextField, InputAdornment, Pagination, Button, CircularProgress, Dialog, DialogTitle, DialogContent, DialogActions } from "@mui/material";
...
const MasterSheetTable = ({ onEdit }) => {
    ...
    const renderDetailSection = (title, fields) => (
        <Box sx={{ mb: 3 }}>
            <Typography variant="subtitle1" sx={{ fontWeight: 'bold', mb: 1, color: 'primary.main', borderBottom: '1px solid #eee', pb: 0.5 }}>{title}</Typography>
    ...
    return (
        <Box>
            <Box sx={{ mb: 3, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 2 }}>
                <TextField placeholder="Search Company, Certificate, Person..." variant="outlined" size="small" onChange={handleSearchChange} sx={{ width: { xs: '100%', sm: 300 } }} InputProps={{ startAdornment: (<InputAdornment position="start"><FiSearch /></InputAdornment>), }} />
                <Box sx={{ display: 'flex', gap: 1 }}>
                    <Button variant="outlined" startIcon={<FiDownload />} size="small" onClick={() => handleExport('xlsx')}>Export Excel</Button>
                    <Button variant="outlined" startIcon={<FiDownload />} size="small" onClick={() => handleExport('csv')}>Export CSV</Button>
                </Box>
            </Box>
            ...
            <TableContainer component={Paper} elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 2, overflow: 'auto' }}>
                <Table sx={{ minWidth: 2800 }} aria-label="master sheet table">
                    ...
                    <TableBody>
                        {loading ? (
                            <TableRow><TableCell colSpan={22} align="center" sx={{ py: 10 }}><CircularProgress size={40} /></TableCell></TableRow>
                        ) : records.length === 0 ? (
                            <TableRow><TableCell colSpan={22} align="center" sx={{ py: 10 }}><Typography variant="body1" color="textSecondary">No records found</Typography></TableCell></TableRow>
                        ) : (
                            records.map((record) => (
            ...
            <Box sx={{ mt: 3, display: 'flex', justifyContent: 'center' }}>
                <Pagination count={Math.ceil(totalCount / limit)} page={page} onChange={(e, v) => setPage(v)} color="primary" shape="rounded" />
            </Box>
